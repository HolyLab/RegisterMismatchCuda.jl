...
cucartesianindex(a::AbstractMatrix) = CartesianIndex(
    (blockIdx().x - 1) * blockDim().x + threadIdx().x,
    (blockIdx().y - 1) * blockDim().y + threadIdx().y
)
cucartesianindex(a::AbstractArray{<:Any, 3}) = CartesianIndex(
    (blockIdx().x - 1) * blockDim().x + threadIdx().x,
    (blockIdx().y - 1) * blockDim().y + threadIdx().y,
...
function kernel_calcNumDenom_intensity!(
        f_fft::T, f2_fft::T, thetaf_fft::T,
        m_fft::T, m2_fft::T, thetam_fft::T,
        numerator_fft::T, denominator_fft::T
    ) where {T}
...
function kernel_calcNumDenom_pixels!(
        f_fft::T, f2_fft::T, thetaf_fft::T,
        m_fft::T, m2_fft::T, thetam_fft::T,
        numerator_fft::T, denominator_fft::T
    ) where {T}

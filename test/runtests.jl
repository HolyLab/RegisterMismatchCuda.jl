...
@testset "Mismatch 3D test" begin
    for dev in devlist
        device!(dev) do
            ...
            for k in -2:2, j in -3:3, i in -4:4
            ...
            for imsz in ((15, 16), (14, 17))
                for maxshift in ((4, 3), (3, 2))
                    for gridsize in ((3, 3), (2, 1), (2, 3), (2, 2), (1, 3))
                        ...
                        for j in -maxshift[2]:maxshift[2], i in -maxshift[1]:maxshift[1]
                        ...
                        denom = sum(denoms)
                        n = Vector{Int}[size(Apad, i) .- abs.(-maxshift[i]:maxshift[i]) for i in 1:2]
